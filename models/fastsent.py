...
class FastSent(utils.SaveLoad):
    ...
    """
    def __init__(
            self, sentences=None, size=100, alpha=0.025, window=10, min_count=5,
    ...
    def __getitem__(self, sentence):
        ...
        if self.fastsent_mean:
            return array([self.syn0[self.vocab[word.replace(u'\ufeff', '')].index] for word in sentence.split() if (self.__contains__(word.replace(u'\ufeff', '')))]).mean(axis=0)
        else:
            return array([self.syn0[self.vocab[word.replace(u'\ufeff', '')].index] for word in sentence.split() if (self.__contains__(word.replace(u'\ufeff', '')))]).sum(axis=0)
